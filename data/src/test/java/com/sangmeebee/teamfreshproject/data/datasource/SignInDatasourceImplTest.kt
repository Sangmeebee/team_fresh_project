...
class SignInDatasourceImplTest {

    @get:Rule
    val mainDispatcherRule = MainDispatcherRule()

    private lateinit var signInAPI: SignInAPI
    private lateinit var signInDatasource: SignInDatasource

    @OptIn(ExperimentalCoroutinesApi::class)
    @Before
    fun setUp() {
        signInAPI = mockk()
        signInDatasource = SignInDatasourceImpl(signInAPI = signInAPI, dispatcher = mainDispatcherRule.testDispatcher)
    }

    @Test
    fun `회원가입을 성공하면 토큰을 반환한다`() = runTest {
        // given
        val signInEntity = SignInEntity(id = "appdev", password = "Timf1234")
        ...
        coEvery { signInAPI.signIn(signInEntity) } returns tokenEntity
        // when
        val actual = signInDatasource.signIn(signIn = signInEntity.toDomain())
        // then
    ...
    fun `아이디를 잘못 입력하면 해당 예외가 발생한다`() = runTest {
        // given
        val signInEntity = SignInEntity(id = "WrongUser", password = "Timf1234")
        val tokenEntity = TokenEntity(
            success = false,
            code = -1000,
    ...
    fun `비밀번호를 잘못 입력하면 해당 예외가 발생한다`() = runTest {
        // given
        val signInEntity = SignInEntity(id = "appdev", password = "WrongPassword")
        val tokenEntity = TokenEntity(
            success = false,
            code = -1001,
